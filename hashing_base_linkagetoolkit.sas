...
**************************************************************************************
	...
	- If additional PII variables need to be hashed, they should be added to this list;
	...
	3. CLEAN RAW DATA
		...
		* Clean names;
			...
			* Convert to lowercase and strip leading and trailing whitespace;
			fn = strip(lowcase(&first_name));
		    ln = strip(lowcase(&last_name));

			* Remove common prefixes/suffixes from first & last names;
			fn = prxchange('s/^(mr\ |mrs\ |dr\ |ms\ |mr\.|mrs\.|dr\.|ms\.)+//', 1, fn);
			ln = prxchange('s/(\ jr|\ sr|\ jr\.|\ sr\.)+\s*$//', 1, ln);
			...
			* Replace special characters with standard alphabetic characters;
			...
			%macro translate_spec_char;
				...
				%do i = 1 %to %sysfunc(countw(&spec_char));
					...
					fn=translate(fn,&temp_new,&temp_old); 
					ln=translate(ln,&temp_new,&temp_old); 
				%end;
			...
			* Remove all non a to z characters;
			fn = prxchange('s/[^a-z]//', -1, fn);
			ln = prxchange('s/[^a-z]//', -1, ln);
		...
		* Clean SSNs;
			...
			* Make three variables to tag invalid SSNs;
		...
		* Drop intermediate vars and original PII;
		drop ssn_length ssnA ssnB ssnC ssnA_num ssnB_num ssnC_num &first_name &last_name &dob &ssn;
...
*************************************************************************************
	4. HASH CLEANED DATA USING AGREED UPON SALT;
...
*************************************************************************************
	...
	* Inputs:
	...
	- latest_year should be the value of the latest plausible birth year in the data being hashed
	- all other inputs should be the name of the corresponding variable
      ...
      the input should be changed to ssn=social_security. all_vars is a list of all of the fields 
	  with personally identifiable data that need to be hashed. For example all_pii= first_name last_name dob ssn
	;

	%hash(		salt="FAKE2!59U~9668SALT",
