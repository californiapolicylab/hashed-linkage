...
########## Condition sets ##########
# Docstring:
#' Class representing a single condition (i.e. perfect match, fuzzy match) in a hashed linkage. Looks for columns suffixed
#' with `_left` and `_right` for each column invovlved in evaluating this condition. This naming convention is applied
#' automatically in conducting a round of a HashedLinkage.
#' 
#' @param field_name The name of the LinkageField to which the condition will be applied (i.e. the piece of PII
...
Condition <- setRefClass(
  ...
  methods=list(
    evaluate=function(.self, linkage_fields, df){
      "Given a dataframe and set of LinkageFields, actually evaluate the match the Condition specifies. If a fuzzy match,
      ...
      # Get the LinkageField object for this field
      this_linkage_field <- linkage_fields[[.self$field_name]]
      ...
      if(.self$perfect){
        ...
        if(!(this_linkage_field$is_compound)){
          ...
          # Calculate whether the left and right values are equal for each partial, and explicitly columnwise
          # bind them (using simplify=TRUE causes issues if the result has exactly one row)
          partials_equal <- sapply(
            this_linkage_field$partial_fields,
...
# Docstring:
#' Class representing a set of Conditions (or Condition_sets) in a hashed linkage. This is intended to be an abstract
#' class in that
...
N_of_conditions <- setRefClass(
  ...
  methods=list(
    evaluate=function(linkage_fields, df){
      ...
      n_of_helper(n, sapply(conditions, function(x)x$evaluate(linkage_fields, df), USE.NAMES=FALSE))
    }
  )
)
...
All_of_conditions <- setRefClass(
  ...
  methods=list(
    evaluate=function(linkage_fields, df){
      ...
      condition_bools <- sapply(conditions, function(x)x$evaluate(linkage_fields, df), USE.NAMES=FALSE)
...
#' *a single column*. For instance, you would create a single LinkageField object for each of first name, last name,
...
# Docstring:
#' Class representing the configuration of the hashed linkage to be conducted. Encapsulates the datasets themselves, and all information
#' needed to score matches on each PII field. The `round` method is used to conduct a single round of a hashed linkage.
#' 
...
#' 
#' @param linkage_fields A list (not vector) of `LinkageField`s. This list should be comprehensive of all of the PII fields on which
#'   the two datasets will be linked.
HashedLinkage <- setRefClass(
  'HashedLinkage',
  fields=list(left_df='ANY', right_df='ANY', linkage_fields='list'),
  methods=list(
    score=function(.self, joined_df){
      #' Score all of the PII fields used in this hashed linkage. Takes in a dataframe containing two copies of each column needed
      #' across all `LinkageField`s, one suffixed with `_left` and one suffixed with `_right`. This is done as part of the `round`
      #' method; the dataframe passed to this method is the result of joining the two datasets on the `join_keys` for a round.
      
      # Call the scoring function for each linkage field
      scores <- sapply(.self$linkage_fields, function(field) field$scoring_function(joined_df), simplify=FALSE, USE.NAMES=FALSE)
      # Rename them to have _score suffixes so that we can just bind them directly to the dataframe in the round
      # method below
      names(scores) <- sapply(.self$linkage_fields, function(mf) mf$name %>% to_score_colname()) %>% unlist() %>% unname()
      scores
    },
    round=function(.self, join_keys, other_conditions=All_of_conditions(conditions=list()), drop_bad=c()){
      #' Method to conduct a single round of a hashed linkage between two datasets.
      #' @param join_keys list/vector of `LinkageField` names on which the datasets will be joined (i.e. two observations must be
      #'   a perfect match on all of these `LinkageField`s to be considered a match candidate.)
      ...
      timer$start('hashed_linkage')
      print(sprintf('Starting hashed linkage round at %s', Sys.time()))
      
      # Create a named linkage_fields list with the field names as the list entry names
      ...
      # Since we're using this object, we'll make reference to linkage_fields rather than .self$linkage_fields
      # throughout the code
      named_linkage_fields <- list()
      for(field in .self$linkage_fields){
        named_linkage_fields[[field$name]] <- field
      }
      ...
      if(length(err_messages) > 0){
        ...
        stop(paste(c(err_messages, 'Check your datasets and the LinkageFields you are passing to this HashedLinkage'), sep=';'))
      }
      ...
      print(sprintf('Left dataset (as passed to hashed_linkage_round) has %d records.', nrow(left_df)))
      print(sprintf('Right dataset (as passed to hashed_linkage_round) has %d records.', nrow(right_df)))
      ...
      join_key_columns <- named_linkage_fields[join_keys] %>%
      ...
      scores <- .self$score(joined)
      joined <- bind_cols(joined, scores)
      ...
      if(has_other_conditions){
        ...
        meets_other_conditions <- other_conditions$evaluate(named_linkage_fields, joined)
        # Filter to candidate matches that meet the required other conditions
        matches <- joined %>% filter(meets_other_conditions)
      }else{
        # Anything that joined on the join keys is a match
        matches <- joined
      }
      ...
      timer$stop('hashed_linkage')
      elapsed_time <- timer$getEvent('hashed_linkage')$timeElapsed
      cat(sprintf('Hashed linkage round completed at %s.\n%d matches found (%d after deduplication).\n%s elapsed.\n',
                  Sys.time(),
                  nrow(matches),
...
# This section contains the default scoring functions and LinkageFields for CPL's default linkage strategy.
# See the readme for more detail on the strategy itself.


# Scoring functions for the four default LinkageFields
...
default_hashed_linkage_object <- function(left_df, right_df, fn=TRUE, ln=TRUE, ssn=TRUE, dob=TRUE){
  #' Function to return a HashedLinkage object corresponding to the default linkage strategy, given two datasets.
  #' Can also subset the default fields using the four flags in the signature. Note that this just returns a
  #' HashedLinkage object; it does not actually run a hashed linkage, or specify anything about the default rounds.
  #' See the `run_default_cpl_hashed_linkage` function to do this.
  linkage_fields <- list()
  if(fn){linkage_fields[['fn']] <- DEFAULT_FN_LINKAGE_FIELD}
  if(ln){linkage_fields[['ln']] <- DEFAULT_LN_LINKAGE_FIELD}
  if(ssn){linkage_fields[['ssn']] <- DEFAULT_SSN_LINKAGE_FIELD}
  if(dob){linkage_fields[['dob']] <- DEFAULT_DOB_LINKAGE_FIELD}
  HashedLinkage(
    left_df=left_df,
    right_df=right_df,
...
write_file_hashed_linkage <- function(df, output_name, ftype, dir='./',
...
write_csv_hashed_linkage <- function(...){
  write_file_hashed_linkage(..., ftype='csv')
}

write_rds_hashed_linkage <- function(...){
  write_file_hashed_linkage(..., ftype='rds')
}


########## The star(s) of the show ##########

run_default_cpl_hashed_linkage <- function(left, right, left_name, right_name,
  ...
  #' if this is desired, it can be done straightforwardly using the LinkageField and HashedLinkage objects.
  ...
  # Define the hashed linkage object
  hashed_linkage <- default_hashed_linkage_object(left_df=left, right_df=right)

  # Round 0: Perfect match on all fields
  round_0 <- hashed_linkage$round(join_keys=c('ssn', 'fn', 'ln', 'dob'), drop_bad=c('bad_ssn', 'bad_fn', 'bad_ln', 'bad_dob'))
  
  # Round 1: Perfect match on ssn, perfect match on 2 of fn/ln/dob, and fuzzy
  # match on whichever didn't perfect match
  round_1 <- hashed_linkage$round(join_keys=c('ssn'), other_conditions=all_of_conditions(
...
generate_match_outputs <- function(all_rounds, left_name, right_name, write_crosswalk=TRUE,
  ...
  save_closure <- function(df, output_name){
    ...
    write_csv_hashed_linkage(df, paste(left_name, right_name, output_name, sep='_'), dir=dir,
